...
async function bookAppointment() {
  ...
  const hasConflict = Array.isArray(busy) && busy.some(
    b => slotStart < new Date(b.end) && new Date(b.start) < slotEnd
  )
  ...
  <div class="container1">
    <div id="child">
      ...
      <div class="book-btn-row">
        <button type="button" class="book-btn" @click="bookAppointment" :disabled="appointmentStore.isLoading || appointmentStore.isUpdating || !currentTimeslot || !picked">
          {{ (appointmentStore.isLoading || appointmentStore.isUpdating) ? (editMode ? 'Updating...' : 'Booking...') : (editMode ? 'Update appointment' : 'Book appointment') }}
        </button>
      </div>
    </div>
  </div>
