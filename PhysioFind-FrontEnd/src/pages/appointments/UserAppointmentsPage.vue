...
function editAppointment(appt: AppointmentRequest) {
  ...
  const startTime = appt.preferred_start ? new Date(appt.preferred_start).toISOString().slice(11, 19) : ''
...
async function confirmDelete() {
  ...
  if (success) {
    ...
    bannerTimer = setTimeout(() => { showDeletedBanner.value = false }, 3000)
    if (appointmentStore.appointments.filter(a => new Date(a.preferred_start ?? 0) > new Date()).length === 0) {
...
<template>
  <div class="deleted-banner" v-if="showDeletedBanner">
  ...
  <div class="container1" v-if="profile">
    <div id="child">
      ...
      <div id="child" v-show="Date.now() < (appt.preferred_start ? new Date(appt.preferred_start).getTime() : 0)" v-for="appt in appointmentStore.appointments" :key="appt.id" :value="appt.id">
        <h3>
          {{ appt.preferred_start ? formatDate(appt.preferred_start) : 'Unknown date' }} — Appointment with {{ appt.practitioner_id ? (practitionerNames[appt.practitioner_id] || 'Loading...') : 'Unknown practitioner' }} from {{ appt.preferred_start ? formatTime(appt.preferred_start) : '' }} to {{ appt.preferred_end ? formatTime(appt.preferred_end) : '' }}
        </h3>
        <div class="buttons-row">
          <button type="button" class="appt-btn edit-btn" @click="editAppointment(appt)" :disabled="appointmentStore.isDeleting">
            Edit appointment
          </button>
          <button type="button" class="appt-btn delete-btn" @click="promptDelete(appt.id)" :disabled="appointmentStore.isDeleting">
            Delete appointment
          </button>
        </div>
      </div>
  ...
  <div v-if="confirmDeleteId" class="modal-overlay">
    <div class="confirm-modal">
      ...
      <div class="buttons-row">
        <button type="button" class="appt-btn edit-btn" @click="cancelDelete">
          No
        </button>
