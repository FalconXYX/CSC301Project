Meeting on February 8, 2026 at 7:30 PM
Lasted 40 minutes
Attendees: @everyone 
Notes
The primary focus of the meeting was sprint planning and assigning issues to group members. 


Meeting on February 16, 2026 at 7:00 PM
Lasted 27 minutes
Attendees: @everyone 
Notes:
General progress update meeting.
Drew will be starting his portion in the next few days, will have it done by Thursday at the latest.
Martin and Parth have to wait until Drew is done but will get started immediately after.
Martin is almost done converting front end to view. Result page format will be changing to include a map.
Jordan will be familiarizing himself with docker before continuing.


Good Practices:
- We had daily stand ups consistently and everyone participated
- Pull Requests were reviwed within 24 hours
- Breaked tasks into subtasks in github project tracking
- Split up worked and assigned tickets very early

Bad Practices:
- Sometimes .env with sensitive info was pushed
- Supa base was made on a temporary account so we had to migrate it to a permanent one